...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 25 17:27:09 2019
...
def word_to_braille(word):
    ...
    if (is_all_caps(word) and len(word) > 1):
        word_in_braille += (CAPITAL + '\n\n' + CAPITAL + '\n\n')
        for char in word:
            word_in_braille += char_to_braille(char) + '\n\n'
    else:
        for char in word:
            if (is_digit(char)):
                word_in_braille += NUMBER +'\n\n'
                word_in_braille += char_to_braille(char) + '\n\n'
            elif (is_capitalized(char)):
                word_in_braille += CAPITAL + '\n\n'
                word_in_braille += char_to_braille(char) + '\n\n'
            elif (is_known_character(char)):
                word_in_braille += char_to_braille(char) + '\n\n'
                
    return word_in_braille
...
def new_filename(fname, addition):
    ...
    fname = fname.replace('.', '_' + addition + '.')
