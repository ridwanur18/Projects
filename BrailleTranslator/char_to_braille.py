...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 12 23:27:10 2019
...
def convert_punctuation(c):
    ...
    if (is_punctuation(c) == True):
        for index, char in enumerate(PUNCTUATION):
            if (c == char):
                return (decade_ending(0) + '\n' + decade_pattern(index))
...
def convert_letter(c):
    ...
    if (is_letter(c) == True):
        braille_from_letter = (decade_pattern(letter_column(c)) + '\n' + decade_ending(letter_row(c)))
        return braille_from_letter
